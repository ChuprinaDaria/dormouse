...
### Assets — lazy download
Великі файли НЕ в пакеті, завантажуються при першому використанні:
- `lexicon.db` (12MB) — GitHub Releases / HuggingFace
- `expr_seq2seq.pt` (28MB) — GitHub Releases / HuggingFace
- `expr_vocab_*.json`, `expr_config.json` — разом з моделлю

Логіка в `src/dormouse/assets.py`:
1. `DORMOUSE_DATA_DIR` env var → dev mode (шукає в локальній `data/`)
2. `~/.cache/dormouse/v{version}/` → cache
3. Download: GitHub Releases → HuggingFace fallback
4. `DORMOUSE_OFFLINE=1` → заборона download

### Що бандлиться в pip пакет (29MB wheel)
- `src/dormouse/data/replacements.json` (44KB)
- `src/dormouse/data/lexicon.db` (12MB)
- `src/dormouse/data/expr_seq2seq.pt` (28MB) + vocab + config
- `pip install dormouse-ua` → одразу працює, нічого не треба качати
...
- `src/dormouse/models/` — тепер в GitHub Releases / HF (lazy download)
